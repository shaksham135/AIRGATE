...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT COALESCE(SUM(p.amount), 0.0) FROM Payment p WHERE p.status = 'SUCCESS' AND p.updatedAt >= :since")
    Double findRevenueSince(@Param("since") LocalDateTime since);
